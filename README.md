...
## Pi Camera
![alt text](Python/Camera_Images/cartoon_snow_.jpg)
![alt text](Python/Camera_Images/watercolor_textbooks_.jpg)
### Reflection
In this assignment, I learned to use the pi camera.  First, I wrote a simple program which took 1 picture and saved it to the Camera_Images folder inside of my Python folder.  The hardest part here was just figuring out how to give each picture a unique name so that they wouldn't override each other.  To do this, I used the "now" function from the "datetime" library to give each picture a unique timestamp.  In the second part of the assignment, I wrote a program which takes 6 pictures at one time, one without any effects, and 5 with varying effects, from a watercolor painting to a cartoon to "emboss", which creates a black and white image that highlights depth.  In this program, I found that the timestamps, while unique, made it heard to find out which picture was which in GitHub, so I instead created a user input which allows the user to name each set of pictures, and then identified each picture by its effect.
## Copypasta 1 - Parent Detector
![alt text](images/Parent_Detector.png) 
### Wiring
![alt text](images/Parent_Detector_Wiring.png)
### Reflection
In this assignment, I used the PIR (Passive InfraRed) motion sensor and the pi camera to create a motion activated video recorder, kind of like a security camera.  Using the motion sensor was extremely simple: all I had to do was create a while loop which paused at the start of each loop until motion was detected.  Interestingly, I found that the motion sensor was far too sensitive and didn't work well when I positioned it too close to my body, likely because the PIR motion sensor works by detecting heat changes, and it was picking up random minor changes in my body heat.  When recording the videos, I decided to make each one 10 seconds long, rather than waiting until motion stopped, because when I used wait_for_no_motion like the assignment suggested I found that the videos were far too short to pick up anything.  The hardest part of this assignment was converting the video format.  The raspberry pi records videos in .h264 format, but this is a relatively rare format which cannot be played on most computers.  In order to convert the videos to .mp4 format, I had to first install the gpac library on my pi, which contains the MP4 box commands that facilitate conversion, and then to make this conversion happen automatically within the python script, I had to use the subprocess library to call linux commands from within a script.
## Copypasta 2 - Stop Motion
![alt text](images/StopMotion_WombatScreenshot.png) 
### Reflection
In this assignment, I used the pi camera to create a stop motion video, composed of a series of images.  The assignment suggested using a push button to take each image, but I did not have one, so I just used the input() function to do the same thing with the enter button on my keyboard.  Like in the last assignment, I also used the subprocess.call() function to perform the linux commands required to combine the pictures into a video from within the Python script.  The final stop motion video, which is saved in the Python/Animation folder on GitHub, is an attempt to depict a clay wombat eating leaves on a plant but being threatened by a dinosaur.  The camera preview doesn't work in the SSH app on the chromebook so I didn't always have the right camera angle in the pictures, but the stop motion story is mostly understandable and the variety of camera angles accurately reflects the chaos of the video.
